...
new #[Title("La Carte - Wendy's Diner")] class extends Component
{
    ...
    // Redeeming Reward
    #[Url]
    public ?int $redeem = null;
    #[Url]
    public ?int $points = null; // Capture points cost from URL
    ...
    public function mount(): void
    {
        ...
        // Auto-open modal if redeeming
        if ($this->redeem) {
            $product = Product::find($this->redeem);
    ...
    {
        // Reset redeeming state when clicking normal products
    ...
    public function addBurgerAsSolo(): void
    {
        ...
        if ($this->isRedeeming) {
             app(CartService::class)->add([
                'id' => 'reward_' . $this->selectedBurger->id . '_' . time(),
                'name' => $this->selectedBurger->name . ' (Récompense)',
                'price' => 0,
                'quantity' => 1,
                'is_menu' => false,
                'is_reward' => true,
                'points_cost' => $this->points, // Save points cost
                'notes' => $this->itemNotes,
                'product_id_for_db' => $this->selectedBurger->id,
            ]);
            $this->dispatch('cart-updated');
            $this->dispatch('notify', message: 'Récompense ajoutée au panier !');
            $this->closeMenuModal();
            return;
        }
    ...
    public function addMenuToCart(): void
    {
        ...
        if ($this->isRedeeming) {
             app(CartService::class)->add([
                'id' => 'reward_menu_' . $menuId, // Unique ID
                'name' => 'Menu ' . $burger->name . ' (Récompense)',
                'price' => 0, // FREE
                'quantity' => 1,
                'is_menu' => true,
                'is_reward' => true,
            ...
            $this->dispatch('cart-updated');
            $this->closeMenuModal();
            $this->dispatch('notify', message: 'Menu récompense ajouté au panier !');
            return;
        }
    ...
    public function closeMenuModal(): void
    {
        ...
        $this->itemNotes = '';
        // Note: We do NOT reset isRedeeming here because if they close by accident we might want to keep it?
        // Actually, if they close, they probably cancel the reward selection.
        // But since we redirect them here with ?redeem=X, the mount() will re-trigger it if we reload.
        // If they just close the modal, let's reset it so they can browse normally.
        $this->isRedeeming = false;

        // Remove query param from URL to prevent reopen on refresh (Livewire feature)
        $this->redeem = null;
    }
